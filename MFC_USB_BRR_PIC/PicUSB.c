  ...
  while (TRUE) {
      if(usb_enumerated())          //si el PicUSB sigue configurado
      {
         if (usb_kbhit(1))          //si el endpoint de salida contiene datos del host
         {
            ...
            usb_get_packet(1, dato_l, 1); //cojemos el paquete de tamaño 8 bits de "dato" y almacenamos en recibe
            if (dato_l[0] == 0) 
            {
            ...
            //MOTOR A PASOS 
           usb_get_packet(1, dato_m, 3 ); //cojemos el paquete de tamaño 8 bits de "dato1" y almacenamos en recibe
           //IZQUIERDA
           if (dato_m[0] == 0x01){
               output_high(PIN_B0);
           }
           ...
           usb_put_packet(1,valor_adc,1,USB_DTS_TOGGLE);
         }//KBHIT
      }//ENUMERATED
   }//WHILE
}//MAIN
